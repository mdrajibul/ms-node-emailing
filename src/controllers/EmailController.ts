  ...
  async send() {
    ...
    if (validateResult === true) {
      sendMail(mailDomain.from, mailDomain.to, mailDomain.subject, mailDomain.body)
        .then(data => {
          Log.log(`Email sent successfully. Details:: From: ${mailDomain.to} To: ${mailDomain.to}, Subject: ${mailDomain.subject}`);
          this.res.status(200).send('Email sent successfully');
        }).catch(error => {
          Log.error(`Email not sent. Details:: From: ${mailDomain.to} To: ${mailDomain.to}, Subject: ${mailDomain.subject}`);
